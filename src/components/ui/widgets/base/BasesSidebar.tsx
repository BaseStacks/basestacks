      ...
      <div className="flex flex-col gap-2 px-4">
        ...
        <div className="w-full flex flex-col gap-1 mb-4">
          ...
          <Button variant="ghost" asChild>
            <Link to="." search={{ page: "overview" }} className="!px-4 w-full">
              <LayoutDashboard />
      ...
      {tables.map((table) => (
        <div key={table.id} className="flex flex-col gap-1">
          <Button variant="ghost" className="!px-4 w-full !pr-1 group" asChild>
            <Link to="." search={{ page: "table", tableId: table.id }}>
              <Table2 />
          ...
          {expandedTables.includes(table.id) && <ViewList tableId={table.id} />}
        </div>
      ))}
    </div>
  );
