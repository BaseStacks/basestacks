...
export function BasesSidebar({ isOverviewPage = false }: NavigationProps) {
      ...
      <div className="h-16 px-4 flex items-center justify-between">
        <div className="grow">
          <Button size="lg" variant="ghost" className="!px-4">
            <Database className={getTextColorClass(activeBase.color)} />{" "}
            {activeBase.name}
            <ChevronDown />
          </Button>
        </div>
      ...
      <div className="flex flex-col gap-2 px-4">
        ...
        <div className="w-full flex flex-col gap-1 mb-4">
          ...
          <Button variant="ghost" asChild>
            <Link to="/overview" className="!px-4 w-full">
              <LayoutDashboard />
        ...
        <div className="w-full flex flex-col gap-1">
          <Label className="px-4 mb-2">Tables</Label>
          <TableList isOverviewPage={isOverviewPage} />
        </div>
      </div>
    </div>
  );
...
function TableList({ isOverviewPage = false }: NavigationProps) {
  const { tables, activeTableId: activeBaseId } = useTables();

  const [expandedTables, setExpandedTables] = useState<Array<string>>(() =>
    ...
    <div className="flex flex-col gap-1">
      {tables.map((table) => (
        <div key={table.id} className="flex flex-col gap-1">
          <Button variant="ghost" className="!px-4 w-full !pr-1 group" asChild>
            <Link
              to={isOverviewPage ? "/" : "."}
          ...
          {expandedTables.includes(table.id) && (
            <ViewList tableId={table.id} isOverviewPage />
          )}
        </div>
      ))}
    </div>
...
}: ViewListProps & NavigationProps) {
  const { getViewsByTableId } = useViews();

  const views = useMemo(
    () => getViewsByTableId(tableId),
    [getViewsByTableId, tableId]
  );

  return (
    <div className="flex flex-col gap-1">
      <Button variant="ghost" className="!px-10 w-full">
